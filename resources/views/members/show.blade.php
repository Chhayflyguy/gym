            ...
            <div class="space-y-8">
                                ...
                                <div class="border-b border-gray-700 last:border-b-0">
                                    <button @click="openExercise = (openExercise === '{{ $exerciseName }}' ? '' : '{{ $exerciseName }}')" class="w-full flex justify-between items-center p-6 text-left hover:bg-gray-700/50 focus:outline-none">
                                        <div class="flex-grow">
                                            ...
                                            <p class="text-sm text-gray-400">{{ $totalSets }} sets &bull; {{ number_format($totalVolume, 0) }} kg total volume</p>
                                        </div>
                ...
                @empty
                    <div class="bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-10 text-center">
                        <p class="text-gray-400">{{ $user->name }} hasn't logged any workouts yet.</p>
                    </div>
                @endforelse
            </div>
        </div>
